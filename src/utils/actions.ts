...
import { caclulateTotals } from "./calculateTotals";
...
const getAdminUser = async () => {
  ...
  if (user.id !== process.env.ADMIN_USER_ID) redirect("/profile/create");
  return user;
};
...
export const fetchProperties = async ({
  ...
  const propertyies = await db.property.findMany({
    where: {
      category,
      OR: [
...
export const deleteReviewAction = async (prevState: { reviewId: string }) => {
  ...
  try {
    ...
    return { message: "review deleted succussfully" };
  } catch (error) {
    return renderError(error);
  }
};
...
export const createBookingAction = async (prevState: {
  ...
  const { orderTotal, totalNights } = caclulateTotals({
    checkIn,
    checkOut,
